...
뽑은 인형을 담을 리스트 q를 만들었다.

num에 현재 뽑힐 예정인 인형의 정보를 담고 있는데,  그 값이 q의 마지막 값과 같으면 q에 있는 값을 삭제해주고 answer에 2를 더해준다.

num이 0이 아닐때는 뽑힐 인형이 존재한다는 것이기 때문에 인형을 뽑을수 있는 상태이다.

그 다음에는 q.size를 비교해준다. 큐가 0인 경우에는 비교없이 인형을 담아준다.

q가 0이 아니라면 q의 끝에 들어있는 인형과 현재 인형 정보를 담고 있는 num을 비교해준다. 

두 수가 같은 경우 q에서 인형을 삭제하고 answer에 2를 더해준다. 
